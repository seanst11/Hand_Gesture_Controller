...
def main():
    ...
    mouse_id_history = deque(maxlen=20)

    # 靜態手勢最常出現參數初始化
    keypoint_length = 8
    keypoint_R = deque(maxlen=keypoint_length)
    keypoint_L = deque(maxlen=keypoint_length)
    m_R = deque(maxlen=keypoint_length)
    m_L = deque(maxlen=keypoint_length)
    ...
    # ========= 主程式運作 =========
    while True:
        left_id = right_id = m_right_id = m_left_id = -1
        fps = cvFpsCalc.get()
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                if handedness.classification[0].label[0:] == 'Right':
                    keypoint_R.append(hand_sign_id_R)
                    most_kR_id = Counter(keypoint_R).most_common()
                    right_id = most_kR_id[0][0] if most_kR_id[0][1] >= 6 else -1

                    m_R.append(mouse_id)
                    most_mR_id = Counter(m_R).most_common()
                    m_right_id = most_mR_id[0][0] if most_mR_id[0][1] >= 6 else -1

                else:
                    keypoint_L.append(hand_sign_id_L)
                    most_kL_id = Counter(keypoint_L).most_common()
                    left_id = most_kL_id[0][0] if most_kL_id[0][1] >= 6 else -1

                    m_L.append(mouse_id)
                    most_mL_id = Counter(m_L).most_common()
                    m_left_id = most_mL_id[0][0] if most_mL_id[0][1] >= 6 else -1
                ...
                debug_image = draw_info_text(
                    ...
                    keypoint_classifier_labels[hand_sign_id_L],
        ...
        # 偵測是否有手勢 #########################################
        if left_id + right_id + m_right_id + m_left_id > -4:
            # change mode Gesture six changes to the different mode
            if most_common_ms_id[0][0] == 3 and most_common_ms_id[0][1] == 20:
                ...
                if time.time() - presstime > 1:
                    ...
                    control_keyboard(left_id, -1, right_id, 0, 'right', keyboard_TF=True, print_TF=False)
                    control_keyboard(left_id, -1, right_id, 7, 'left', keyboard_TF=True, print_TF=False)
                ...
                elif m_right_id == 1 or m_left_id ==1:
